...
export function servicoLogarUsuario(login) { return servidor.post("/usuarios/login", login); };
...
export function servicoAlterarUsuario(usuario) {
    return servidor.patch("/usuarios/alterar-usuario",
        usuario, { headers: { Authorization: `Bearer ${usuario.tokenRecuperacao}` } });
};
export function servicoRemoverUsuario(cpf) { return servidor.delete(`/usuarios/${cpf}`); };
